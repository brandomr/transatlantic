...
def populate():
	...
	Ebola_cat = add_cat('Ebola', 128, 64, "Notes on the ongoing Ebola outbreak in West Africa and efforts to contain it.")
	...
	add_page(cat=Ebola_cat,
		...
		views=10,
		drafter=User.objects.get(username="brandomr")
		)
	...
	IsPal_cat = add_cat("Israel Palestine Conflict", 64, 32, "The ongoing conflict between Israel and Hamas as well as the negotiations for peace.")
	...
	EasternUK_cat = add_cat("Rebels in Eastern Ukraine", 32, 16, "Clashes in Eastern Ukraine between the Government of Ukraine and the rebel-separatists.")
...
def add_page(cat, title, url, content, drafter, views=0):
	p = Page.objects.get_or_create(category=cat, title=title, url=url, content=content, views=views, drafter=drafter)[0]
	return p

def add_cat(name, views, likes, description):
	c = Category.objects.get_or_create(name=name, views=views, likes=likes, description=description)[0]
	return c

#start execution here!
if __name__ == '__main__':
	print "Starting transatlantic population script..."
	os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'transatlantic_project.settings')
	from transatlantic.models import Category, Page
